## 更新日志
- v1.8.5
  1. 增加账号启用/停用开关，停用账号不再参与用量刷新、智能切换和 API 反代候选。
  2. 增加 API 反代健康看板，便于观察候选账号、请求耗时和错误分布。
  3. 优化代理与用量刷新失败处理，避免自动刷新/重试把临时失败写成“需要重新登录”。
  4. 保持 GPT-5.5 大上下文能力，并支持在设置中配置上下文窗口与自动 compact 阈值。
- v1.8.4
